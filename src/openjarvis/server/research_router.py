...
async def research(req: ResearchRequest, request: Request) -> StreamingResponse:
    ...
    """
    # Research runs on the server's configured model (not the per-request chat
    # selector), falling back to the configured default and finally the built-in
    # planner default. This keeps it on a model that is actually available
    # rather than a hardcoded one the user may not have pulled.
    cfg = getattr(request.app.state, "config", None)
    planner_model = (
        getattr(request.app.state, "model", None)
        or (cfg.intelligence.default_model if cfg else None)
